...
import './index.css'
import MatchGameTabItems from '../MatchGameTabItems'
...
class MatchGame extends Component {
  ...
  updateTabId = id => {
    this.setState({initialTabId: id})
  }

  onDisplayCategoryItem = () => {
    const {initialTabId} = this.state
    const categoryItems = imagesList.filter(
      each => each.category === initialTabId,
    )
  ...
  renderGame = () => {
    const {initialImage, initialTabId} = this.state
    const {imageUrl} = initialImage
    const displayCategoryItems = this.onDisplayCategoryItem()
    return (
        ...
        <ul className="tabItems-container">
          {tabsList.map(eachTab => (
            <MatchGameTabItems
              ...
              isActive={eachTab.tabId === initialTabId}
              key={eachTab.tabId}
            />
          ))}
        </ul>
        <ul className="thumbnails-images-container">
          {displayCategoryItems.map(eachImageDetails => (
            <MatchGameItem
              eachImageDetails={eachImageDetails}
              isMatchImage={this.isMatchImage}
  ...
  onRest = () => {
    ...
    this.timerId = setInterval(this.setTime, 1000)
    this.setState({initialScore: 0, initialSeconds: 60})
  }

  render() {
    const {initialSeconds, initialScore, isMatch} = this.state
    return (
      <div className="container">
        <div className="navbar-container">
          ...
          <div className="navbar-score-time-container">
            <ul className="score-head-container">
              <li className="score-head-list-container">
                <p className="score">Score: {initialScore}</p>
              </li>
              <div className="countdown-container">
                <li className="countdown-head-list-container">
                  <img
                    src="https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png"
                    className="timer"
                    alt="timer"
                  />
                  <p className="time-countdown-text">{initialSeconds} sec</p>
                </li>
              </div>
            </ul>
          </div>
        </div>
        {!isMatch && this.renderGame()}
        {isMatch && (
          <ScoreCard initialScore={initialScore} onRest={this.onRest} />
        )}
      </div>
    )
  }
